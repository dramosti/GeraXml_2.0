...
namespace HLP.bel.CTe
{
    public class belCriaXml
    {
        ...
        public belCriaXml(X509Certificate2 cert)
        {
            this.cert = cert;
        }
        public belCriaXml()
        ...
        public List<belStatusCte> GerarXmlCancelamento(belCancelaCte objCancelaCte)
        {
            try
            {
                ...
                XContainer cancCTe = new XElement(pf + "cancCTe", new XAttribute("versao", "1.04"),
                ...
                belAssinaCte Assinatura = new belAssinaCte();
                string sInfCanc = Assinatura.ConfigurarArquivo(cancCTe.ToString(), "infCanc", this.cert);
                ...
                try
                {
                    ...
                    reader = new XmlValidatingReader("<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + xInfCanc.ToString(), XmlNodeType.Element, context);

                    myschema.Add("http://www.portalfiscal.inf.br/cte", belStaticPastas.SCHEMA_CTE + "\\cancCte_v1.04.xsd");
        ...
        private static XmlDocument MontaXmlConsultaSituacao(string sChave)
        {
            ...
            XContainer consReciCTe = new XElement(pf + "consSitCTe", new XAttribute("versao", "1.04"),
            ...
            try
            {
                ...
                myschema.Add("http://www.portalfiscal.inf.br/cte", belStaticPastas.SCHEMA_CTE + "\\consSitCte_v1.04.xsd");
        ...
        public string TransmitirLote(XmlDocument doc)
        {
            ...
            try
            {
                ...
                {
                    HLP.WebService.CTe_Homologacao_cteRecepcao.cteCabecMsg objCabecRecepcao = new HLP.WebService.CTe_Homologacao_cteRecepcao.cteCabecMsg();
                    objCabecRecepcao.cUF = objUf.CUF;
                    objCabecRecepcao.versaoDados = "1.04";

                    HLP.WebService.CTe_Homologacao_cteRecepcao.CteRecepcao objRecepcao = new HLP.WebService.CTe_Homologacao_cteRecepcao.CteRecepcao();
                    objRecepcao.cteCabecMsgValue = objCabecRecepcao;
                    objRecepcao.ClientCertificates.Add(cert);
                    sRetorno = objRecepcao.cteRecepcaoLote(doc).OuterXml;
                }
                //Produção
                else if (belStatic.TpAmb == 1)
                {
                    HLP.WebService.CTe_Producao_cteRecepcao.cteCabecMsg objCabecRecepcao = new HLP.WebService.CTe_Producao_cteRecepcao.cteCabecMsg();
                    objCabecRecepcao.cUF = objUf.CUF;
                    objCabecRecepcao.versaoDados = "1.04";

                    HLP.WebService.CTe_Producao_cteRecepcao.CteRecepcao objRecepcao = new HLP.WebService.CTe_Producao_cteRecepcao.CteRecepcao();
                    objRecepcao.cteCabecMsgValue = objCabecRecepcao;
                    objRecepcao.ClientCertificates.Add(cert);
                    sRetorno = objRecepcao.cteRecepcaoLote(doc).OuterXml;
                }
        ...
        public string CancelarCte(XmlDocument doc)
        {
            ...
            try
            {
                ...
                if (belStatic.TpAmb == 2)
                {
                    HLP.WebService.CTe_Homologacao_cteCancelamento.cteCabecMsg objCabecCancelamento = new HLP.WebService.CTe_Homologacao_cteCancelamento.cteCabecMsg();
                    objCabecCancelamento.cUF = objUf.CUF;
                    objCabecCancelamento.versaoDados = "1.04";

                    HLP.WebService.CTe_Homologacao_cteCancelamento.CteCancelamento objCancelamento = new HLP.WebService.CTe_Homologacao_cteCancelamento.CteCancelamento();
                    objCancelamento.cteCabecMsgValue = objCabecCancelamento;
                    objCancelamento.ClientCertificates.Add(cert);
                    sRetorno = objCancelamento.cteCancelamentoCT(doc).OuterXml;
                }
                //Produção
                else if (belStatic.TpAmb == 1)
                {
                    HLP.WebService.CTe_Producao_cteCancelamento.cteCabecMsg objCabecCancelamento = new HLP.WebService.CTe_Producao_cteCancelamento.cteCabecMsg();
                    objCabecCancelamento.cUF = objUf.CUF;
                    objCabecCancelamento.versaoDados = "1.04";

                    HLP.WebService.CTe_Producao_cteCancelamento.CteCancelamento objCancelamento = new HLP.WebService.CTe_Producao_cteCancelamento.CteCancelamento();
                    objCancelamento.cteCabecMsgValue = objCabecCancelamento;
                    objCancelamento.ClientCertificates.Add(cert);
                    sRetorno = objCancelamento.cteCancelamentoCT(doc).OuterXml;
                }
        ...
        public string InutilizaFaixaCte(XmlDocument doc)
        {
            ...
            try
            {
                ...
                if (belStatic.TpAmb == 2)
                {
                    HLP.WebService.CTe_Homologacao_cteInutilizacao.cteCabecMsg objCabecInutiliza = new HLP.WebService.CTe_Homologacao_cteInutilizacao.cteCabecMsg();
                    objCabecInutiliza.cUF = objUf.CUF;
                    objCabecInutiliza.versaoDados = "1.04";

                    HLP.WebService.CTe_Homologacao_cteInutilizacao.CteInutilizacao objInutilizacao = new HLP.WebService.CTe_Homologacao_cteInutilizacao.CteInutilizacao();
                    objInutilizacao.cteCabecMsgValue = objCabecInutiliza;
                    objInutilizacao.ClientCertificates.Add(cert);
                    sRetorno = objInutilizacao.cteInutilizacaoCT(doc).OuterXml;
                }
                //Produção
                else if (belStatic.TpAmb == 1)
                {
                    HLP.WebService.CTe_Producao_cteInutilizacao.cteCabecMsg objCabecInutiliza = new HLP.WebService.CTe_Producao_cteInutilizacao.cteCabecMsg();
                    objCabecInutiliza.cUF = objUf.CUF;
                    objCabecInutiliza.versaoDados = "1.04";

                    HLP.WebService.CTe_Producao_cteInutilizacao.CteInutilizacao objInutilizacao = new HLP.WebService.CTe_Producao_cteInutilizacao.CteInutilizacao();
                    objInutilizacao.cteCabecMsgValue = objCabecInutiliza;
                    objInutilizacao.ClientCertificates.Add(cert);
                    sRetorno = objInutilizacao.cteInutilizacaoCT(doc).OuterXml;
                }
        ...
        public string ConsultarStatus(XmlDocument doc)
        {
            ...
            try
            {
                ...
                if (belStatic.TpAmb == 2)
                {
                    HLP.WebService.CTe_Homologacao_cteStatusServico.cteCabecMsg objCabecStatus = new HLP.WebService.CTe_Homologacao_cteStatusServico.cteCabecMsg();
                    objCabecStatus.cUF = objUf.CUF;
                    objCabecStatus.versaoDados = "1.04";

                    HLP.WebService.CTe_Homologacao_cteStatusServico.CteStatusServico objStatus = new HLP.WebService.CTe_Homologacao_cteStatusServico.CteStatusServico();
                    objStatus.cteCabecMsgValue = objCabecStatus;
                    objStatus.ClientCertificates.Add(cert);
                    sRetorno = objStatus.cteStatusServicoCT(doc).OuterXml;
                }
                //Produção
                else if (belStatic.TpAmb == 1)
                {
                    HLP.WebService.CTe_Producao_cteStatusServico.cteCabecMsg objCabecStatus = new HLP.WebService.CTe_Producao_cteStatusServico.cteCabecMsg();
                    objCabecStatus.cUF = objUf.CUF;
                    objCabecStatus.versaoDados = "1.04";

                    HLP.WebService.CTe_Producao_cteStatusServico.CteStatusServico objStatus = new HLP.WebService.CTe_Producao_cteStatusServico.CteStatusServico();
                    objStatus.cteCabecMsgValue = objCabecStatus;
                    objStatus.ClientCertificates.Add(cert);
                    sRetorno = objStatus.cteStatusServicoCT(doc).OuterXml;
                }
        ...
        public List<belStatusCte> ConsultaLoteEnviado(string sRecibo)
        {
            ...
            try
            {
                ...
                if (belStatic.TpAmb == 2)
                {
                    HLP.WebService.CTe_Homologacao_cteRetRecepcao.cteCabecMsg objCabecRetorno = new HLP.WebService.CTe_Homologacao_cteRetRecepcao.cteCabecMsg();
                    objCabecRetorno.cUF = objUf.CUF;
                    objCabecRetorno.versaoDados = "1.04";

                    string sConsulta = MontaXmlBuscaRetorno(sRecibo);

                    HLP.WebService.CTe_Homologacao_cteRetRecepcao.CteRetRecepcao objRetRecepcao = new HLP.WebService.CTe_Homologacao_cteRetRecepcao.CteRetRecepcao();
                    objRetRecepcao.cteCabecMsgValue = objCabecRetorno;
                    objRetRecepcao.ClientCertificates.Add(cert);
                ...
                {
                    HLP.WebService.CTe_Producao_cteRetRecepcao.cteCabecMsg objCabecRetorno = new HLP.WebService.CTe_Producao_cteRetRecepcao.cteCabecMsg();
                    objCabecRetorno.cUF = objUf.CUF;
                    objCabecRetorno.versaoDados = "1.04";

                    string sConsulta = MontaXmlBuscaRetorno(sRecibo);

                    HLP.WebService.CTe_Producao_cteRetRecepcao.CteRetRecepcao objRetRecepcao = new HLP.WebService.CTe_Producao_cteRetRecepcao.CteRetRecepcao();
                    objRetRecepcao.cteCabecMsgValue = objCabecRetorno;
                    objRetRecepcao.ClientCertificates.Add(cert);
