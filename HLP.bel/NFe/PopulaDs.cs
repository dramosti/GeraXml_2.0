...
namespace HLP.bel
{
    ...
    {        
        ...
        // datatable Código de Barras
        dsDanfe.codigo_barrasRow drCodigoBarras;

        // datatables da tag IDE  
        dsDanfe.refNFRow drrefNF; //1
        dsDanfe.NFrefRow drNFref; //2
        dsDanfe.ideRow drIde;     //3  

        // datatables da tag EMIT
        dsDanfe.enderEmitRow drenderEmit; //1
        dsDanfe.emitRow dremit; //2

        // datatable da tag AVULSA não utilizada

        // datatable da tag dest
        ...
        // datatables retira e entrega não utilizada

        //datatables da tag det
        ...
        public void populaTagInfNFe(dsDanfe dsdanfe, XmlDocument xml, string CodinfNfeId, int ihoraImpDanfe, int iDataImpDanfe)
        {
           ...
           GeraXMLExp objbelGeraExp = new GeraXMLExp();
            ...
            if (xNinfNFe.Count > 0)
            {
                for (int i = 0; i < xNinfNFe.Count; i++)
                {
                    ...
                    if (dsdanfe.infNFe[Convert.ToInt16(CodinfNfeId) - 1].Id != "") // Diego - OS_24302 - 01/04/2010
                    {
                        ...
                        Byte[] bimagem = belUtil.SalvaCodBarras(dsdanfe.infNFe[Convert.ToInt16(CodinfNfeId) - 1].Id);
                        Byte[] bImagemDadosNfe = belUtil.SalvaCodBarras(sCodDadosNfe);
